 ...
 */
async function killRunningPublishProcesses(): Promise<{ killed: number; detail: string; allStopped: boolean }> {
  ...
  if (process.platform === "win32") {
    ...
    try {
      ...
      return { killed: 0, allStopped: false, detail: `프로세스 조회 실패: ${getErrorMessage(error)}` };
    }

    if (pids.length === 0) {
      return { killed: 0, allStopped: true, detail: "실행 중인 발행 프로세스가 없습니다." };
    }
    ...
    return { killed, allStopped: killed === pids.length, detail: `발행 프로세스 ${killed}/${pids.length}개(및 하위 브라우저)를 종료했습니다.` };
  }

  // POSIX
  try {
    await execFileAsync("pkill", ["-9", "-f", sig]);
    return { killed: 1, allStopped: true, detail: "발행 프로세스를 종료했습니다." };
  } catch {
    return { killed: 0, allStopped: false, detail: "실행 중인 발행 프로세스가 없거나 종료할 수 없습니다." };
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const drafts = kill.allStopped ? await prisma.brandLink.updateMany({
      where: { status: "DRAFTING" },
      data: { status: "FAILED", errorMessage: `${note} 초안 작성 프로세스 종료를 확인했습니다. 소재 준비에서 재개하세요.` },
    }) : { count: 0 };
    const links = await prisma.brandLink.updateMany({
      where: { status: "PUBLISHING" },
      data: { status: "OUTCOME_UNKNOWN", errorMessage: `${note} 제출 여부가 불확실하여 재발행을 차단했습니다. 실제 게시 결과를 먼저 확인하세요.` },
    });
    const tasks = await prisma.topicPostTask.updateMany({
      where: { status: "PUBLISHING" },
      data: { status: "OUTCOME_UNKNOWN", pipelineStage: "OUTCOME_UNKNOWN", errorMessage: `${note} 실제 게시 결과 확인이 필요합니다.` },
    });

    return NextResponse.json({
      success: true,
      message: `포스팅을 정지했습니다. ${kill.detail} 게시 여부 확인 필요: 상품 ${links.count}건, 주제글 ${tasks.count}건.`,
      data: {
        killedProcesses: kill.killed,
        processStopConfirmed: kill.allStopped,
        interruptedDrafts: drafts.count,
        cancelledImageJobs,
