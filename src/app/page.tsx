...
export default function Dashboard() {
    ...
    try {
      ...
      if (payload?.publishMode === "schedule") {
        ...
        if (adjusted && effective) {
          ...
          setDashboardNotice({
            tone: "success",
            text: `예약발행 시작: 예약일 ${effective}${logFile ? ` / 로그: ${logFile}` : ""}`,
          });
        }
      } else if (logFile) {
        setDashboardNotice({
  ...
  const handleSchedulePublish = async (link: BrandLink) => {
    ...
    if (input === null) {
      setDashboardNotice({
        tone: "info",
        text: "예약일 입력이 취소되었거나 브라우저에서 입력창(prompt)이 차단되었습니다.",
      });
      return;
    }
  ...
  const handleBulkSchedulePublish = async () => {
    ...
    if (readyScheduledCount === 0) {
      setDashboardNotice({
        tone: "info",
        text: "예약일이 지정된 대기 링크(READY)가 없습니다.",
      });
      return;
    }
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <p className="text-xs text-slate-500">
                  시즌 등록은 마지막 예약일 다음날부터 10일치로 자동 배치됩니다.
                </p>
              </div>
              {busyMessageForUi && (
                        ...
                        <tr key={link.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 text-center">
                            <div className="space-y-1">
                              ...
                              <div className="text-xs text-slate-500">
                                예약일: {formatDateDisplay(link.scheduledPublishAt)}
                              </div>
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
          ...
          <ol className="text-sm text-slate-600 space-y-1 list-decimal list-inside">
            ...
            <li>⚙️ 시즌 추천 10개 등록 버튼으로 마지막 예약일 다음날부터 10일치 자동 등록</li>
            <li>⚙️ 예약발행 일괄 실행 버튼으로 예약일이 있는 READY 글을 순차 예약발행</li>
            <li>🚀 즉시 버튼은 바로 발행, 📅 예약 버튼은 예약 발행</li>
            <li>예약일이 과거 또는 당일이면 자동으로 다음날로 조정됩니다.</li>
          </ol>
        </div>
      </main>
