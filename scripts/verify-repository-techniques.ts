...
const product = {
  ...
  features: [
    ...
    "타프 걸이형",
    "3단 풍량",
    "구매후기 근거: 바닥 공간을 차지하지 않아 텐트 안이 정돈된다는 점이 좋았어요",
  ],
  price: "29,800원",
  targetSectionCount: 12,
};
...
assert.equal(plan.sections[6]?.role, "review-evidence");
assert.equal(plan.sections[8]?.role, "limitations");
...
const sectionBodies = [
  ...
  ["타프 걸이형 구조는 팬을 상부에 고정해 바닥 공간을 비우는 방식이에요.", "무선 전원은 케이블을 길게 끌지 않고도 원하는 위치에 설치하게 해줍니다.", "3단 풍량은 취침과 식사처럼 상황이 달라질 때 바람 세기를 나누는 기능이에요.", "소음 수치와 배터리 지속시간은 수집 정보에 없어 성능 판단에서 보류합니다."],
  ...
  ["사용 전에는 타프 고리의 고정력과 팬이 흔들리지 않는지 먼저 확인해 설치해요.", "타프 아래 여러 사람이 쉬는 동안에는 1단부터 켜고 필요한 만큼 풍량을 올리면 됩니다.", "사용 후에는 날개 주변 먼지를 닦고 완전히 충전한 뒤 습기 없는 가방에 보관해요.", "한 사람에게 강한 직진풍만 필요한 상황이라면 스탠드형이 더 실용적입니다."],
  ["실제 구매후기에서는 바닥 공간을 차지하지 않아 텐트 안이 정돈된다는 점이 반복돼요.", "사용자들은 장비가 많은 캠핑에서도 이동 통로를 확보하기 쉽다는 점을 장점으로 꼽습니다.", "이 평가는 상부 고정 구조라는 제품 특징과도 직접 연결됩니다.", "다만 수집된 후기 문장 밖의 소음이나 배터리 만족도까지 확대해서 해석하지는 않았어요."],
