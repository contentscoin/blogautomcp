...
ChatGPT에서 초안을 요청하면 PC가 상품 근거와 하네스를 준비하고, 현재 ChatGPT 대화가
원고를 작성한 뒤 PC에 다시 제출합니다. 이 MCP 경로는 `OPENAI_API_KEY`를 사용하지 않습니다.
데스크톱 화면의 단독 초안 생성 버튼을 직접 사용할 때만 선택한 OpenAI/Gemini API 키가 필요합니다.
