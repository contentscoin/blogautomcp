const fs = require('fs');
let code = fs.readFileSync('scripts/topic-agent.ts', 'utf-8');

const regex = /\/\/ ============================================[\s\S]*?async function generateAdvancedContent\([\s\S]*?\n\}/m;

const newCode = `// ============================================
// LLM으로 글 생성 (에이전틱 다단계 고도화 파이프라인 - Opal AI Persona)
// ============================================
  ...
  "subtopics": [
    ...
    { "id": 2, "heading": "소주제 2", "intent": "이 단락에서 전달할 핵심 메시지와 분위기" }
            ...
            try {
                ...
                if (typeof plan !== "object" || !plan.title || !plan.subtopics) throw new Error("Invalid plan format");
            } catch (e) {
...
if (regex.test(code)) {
    ...
    console.log("Successfully replaced generateAdvancedContent");
} else {
